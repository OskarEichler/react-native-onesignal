...
var EventManager = class {
	...
	dispatchHandlers(eventName, payload) {
		const handlerArray = this.eventListenerArrayMap.get(eventName);
		if (handlerArray) handlerArray.slice().forEach((handler) => {
			handler(payload);
		});
		else if (eventName === "OneSignal-notificationWillDisplayInForeground") payload.getNotification().display();
	}
};
//#endregion
...
function encodeNullsForIOS(value) {
	...
	if (typeof value === "object") return Object.fromEntries(Object.entries(value).map(([key, item]) => [key, encodeNullsForIOS(item)]));
	return value;
}
//#endregion
...
if (typeof prev?.clearListeners === "function") prev.clearListeners();
...
async function _addPushSubscriptionObserver() {
	...
	if (version === subscriptionVersion) pushSub = {
		id: id ?? void 0,
		token: token ?? void 0,
		optedIn
	};
...
(function(_OneSignal) {
	function initialize(appId) {
		...
		Promise.all([_addPermissionObserver(), _addPushSubscriptionObserver()]).catch((error) => {
